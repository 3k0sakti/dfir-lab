    ...
    case "$scenario" in
        "1"|"apt")
            ...
            if [ -f "./apt_simulation.sh" ]; then
                docker exec "$CONTAINER_NAME" /tmp/apt_simulation.sh
            else
        ...
        "2"|"ransomware")
            ...
            if [ -f "./ransomware_simulation.sh" ]; then
                docker exec "$CONTAINER_NAME" /tmp/ransomware_simulation.sh
            else
        ...
        "3"|"insider")
            ...
            if [ -f "./insider_threat_simulation.sh" ]; then
                docker exec "$CONTAINER_NAME" /tmp/insider_threat_simulation.sh
            else
        ...
        "4"|"webapp")
            ...
            if [ -f "./webapp_attack_simulation.sh" ]; then
                docker exec "$CONTAINER_NAME" /tmp/webapp_attack_simulation.sh
            else
